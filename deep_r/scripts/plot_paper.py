...
fig, axes = plt.subplots(1, 2, figsize=(double_column_width, 0.8 * column_width), sharey=True)
...
dvs_gesture_accuracy_df = load_data_frame(accuracy_keys, lambda params: params["dataset"] == "dvs_gesture",
                                          path=os.path.join("results", "best_test_no_dalian"),
                                          load_train=True, load_test=True)

accuracy_actors = plot_accuracy_axis(dvs_gesture_accuracy_df, axes[1], accuracy_bar_group_params,
                                     "{(float(hidden_recurrent_sparsity) * 100):.0f}% $W^{{rec}}$\n{(float(hidden_input_sparsity) * 100):.0f}% $W^{{in}}$", 
                                     "100% $W^{rec}$\n100% $W^{in}$")

n_mnist_accuracy_df = load_data_frame(accuracy_keys, lambda params: params["dataset"] == "n_mnist",
                                          path=os.path.join("results", "best_test_no_dalian"),
                                          load_train=True, load_test=True)
plot_accuracy_axis(n_mnist_accuracy_df, axes[0], accuracy_bar_group_params,
                   "{(float(hidden_recurrent_sparsity) * 100):.0f}% $W^{{rec}}$\n{(float(hidden_input_sparsity) * 100):.0f}% $W^{{in}}$", 
                                     "100% $W^{rec}$\n100% $W^{in}$")
